...
from backbone.linears import SimpleLinear, SplitCosineLinear, CosineLinear, SimpleMLP
...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        model.load_state_dict(torch.load(vit_weights_path, map_location='cpu'), strict=False)
        model.out_dim = 768
        return model.eval()
        ...
        model = timm.create_model("resnet152", num_classes=0)
        resnet_weights_path = os.getenv("RESNET_WEIGHTS_PATH", '/mnt/model/I-MER/resnet152.bin')
        model.load_state_dict(torch.load(resnet_weights_path, map_location='cpu'), strict=False)
        from backbone import swin_train
        model = timm.create_model("myswin_train", pretrained=True, num_classes=0)
        model.out_dim = 768
        return model.eval()
    ...
    # SSF (Removed as backbone/vision_transformer_ssf.py is missing and no config uses it)
    ...
    # Adapter (Removed as backbone/vision_transformer_adapter.py is missing and no config uses it)
    # L2P
    elif '_l2p' in name:
    ...
    # dualprompt
        ...
        if args["model_name"] == "dualprompt":
            ...
            raise NotImplementedError("Inconsistent model name and model type")
    # Coda_Prompt (Removed as coda_prompt model is not available)
    else:
...
class SimpleIncreNet(BaseNet):
    def __init__(self, args, pretrained):
        super().__init__(args, pretrained)
        self.weight = None

    def update_fc(self, nb_classes, feature_dim,  nextperiod_initialization=None):
        fc = self.generate_fc(feature_dim, nb_classes).to(self._device)
        if self.fc is not None:
            #fc.sigma.data = self.fc.sigma.data
            if nextperiod_initialization is not None:
                weight = nextperiod_initialization
            else:
                weight = torch.zeros(nb_classes, feature_dim).to(self._device)
            fc.weight = nn.Parameter(weight)
        del self.fc
        self.fc = fc
    ...
    def foward_weight(self, x):
        x = self.backbone(x)
        weightfc = self.generate_fc(self.feature_dim, self.weight.shape[0]).to(self._device)
        weightfc.weight = nn.Parameter(self.weight)
        out = weightfc(x)
        out.update({"features": x})
        return out
...
class SimpleIncreCosNet(BaseNet):
    def __init__(self, args, pretrained):
        super().__init__(args, pretrained)
        # for RanPAC
        self.W_rand = None
    ...
    def extract_vector(self, x):
        with torch.no_grad():
            if self.original_backbone is not None:
                ...
                cls_features = None
        x = self.backbone(x, cls_features=cls_features)
        return x['logits']
