...
class SwinTransformerV2_train(nn.Module):
    """ Swin Transformer V2
...
@register_model
def myswin_train(checkpoint = True,**kwargs):
    model = SwinTransformerV2_train(img_size = 256,window_size = 8, embed_dim = 96, depths = (2, 2, 6, 2), num_heads = (3, 6, 12, 24)
                              , **kwargs)
    swin_weights_path = os.getenv("SWIN_WEIGHTS_PATH", '/mnt/model/I-MER/swinv2_tiny_patch4_window8_256.pth')
    checkpoint_ = torch.load(swin_weights_path, map_location='cpu')
    model_dict = checkpoint_filter_fn(checkpoint_, model)
...
if __name__ == "__main__":
    model = SwinTransformerV2_train(img_size = 256,window_size = 8, embed_dim = 96, depths = (2, 2, 6, 2), num_heads = (3, 6, 12, 24))
    swin_weights_relative_path = os.getenv("SWIN_WEIGHTS_RELATIVE_PATH", 'weight/swinv2_tiny_patch4_window8_256.pth')
    checkpoint_ = torch.load(swin_weights_relative_path, map_location='cpu')
    model_dict = checkpoint_filter_fn(checkpoint_, model)
    model.load_state_dict(model_dict, strict=False)
